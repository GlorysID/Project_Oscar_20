  document.addEventListener("DOMContentLoaded", function () {
    ...
    // Render crop preview (with zoom & offset)
    function renderCropPreview() {
      ...
      // Hitung skala dan offset agar tidak gepeng
      const scale = cropZoom;
      const iw = cropImg.naturalWidth;
      const ih = cropImg.naturalHeight;
      // Skala gambar agar cover area crop (seperti object-fit: cover)
      const aspectImg = iw / ih;
      const aspectCrop = 1; // lingkaran
      let drawW, drawH;
      ...
      // Offset default ke tengah, lalu ditambah cropOffset
    ...
    avatarCropApply.addEventListener("click", function () {
      if (!cropImg) return;
      // Render ke canvas final (220x220)
      const size = 220;
      ...
      // Offset sama seperti preview, tapi perlu scaling ke 220x220
      const offsetX = (previewSize - drawW) / 2 + cropOffset.x;
      const offsetY = (previewSize - drawH) / 2 + cropOffset.y;
      // Draw ke canvas 220x220, scale dari preview
      ctx.drawImage(
        cropImg,
      ...
      // === Tambahan: isi input file dengan hasil crop ===
      canvas.toBlob(function (blob) {
        // Buat file baru dari blob
        const file = new File([blob], "avatar.png", { type: "image/png" });
        // Pakai DataTransfer untuk set input file
      ...
      cropImg = null;
      // avatarInput.value = ""; // jangan direset di sini!
      cropOffset = { x: 0, y: 0 };
    });
    ...
    // === Edit Profile Logic ===
    // (Jika ingin tombol edit, tambahkan <button id="editProfileBtn">Edit Profil</button> di HTML)
    if (editProfileBtn) {
  ...
  avatarInput.addEventListener("change", function () {
    ...
    reader.onload = function (e) {
      ...
      avatarCropModal.classList.add("active"); // modal hanya aktif kalau ada file
    };
    reader.readAsDataURL(file);
  });
